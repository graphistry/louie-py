...
class MockResponse:
    ...
    def __init__(self, include_traces=False):
        ...
        if include_traces:
            ...
            self.trace_elements = [
                {"type": "TraceElement", "content": "Step " + str(i) + ": " + ("x" * 500)}
...
class TestTraceOverhead:
    ...
    def test_memory_usage_with_history(self, mock_client_class):
        ...
        # Measure baseline
        initial_size = sys.getsizeof(cursor._history)
    ...
    def test_dataframe_access_performance(self, mock_client_class):
        ...
        for _ in range(1000):
            start = time.perf_counter()
            df = cursor.df  # Should return first df quickly
            end = time.perf_counter()
            times.append(end - start)
        ...
        for _ in range(1000):
            start = time.perf_counter()
            dfs = cursor.dfs  # Should extract all dfs
            end = time.perf_counter()
            times.append(end - start)
