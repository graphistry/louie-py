...
def test_documentation_file(doc_file: Path):
    ...
    for i, (code, line_num, context) in enumerate(blocks):
        ...
        try:
            ...
            # Patch imports
            with patch('graphistry.register') as mock_register, \
                 patch('graphistry.api_token', return_value="fake-token"), \
                 patch('graphistry.nodes', return_value=Mock(edges=Mock(return_value=Mock()))):
                ...
                if 'import graphistry' in code or 'from louieai import' in code:
                    with patch.dict('sys.modules', {
                        ...
                        'graphistry': Mock(
                            ...
                            nodes=Mock(return_value=Mock(edges=Mock(return_value=Mock())))
                        )
                    }):
                        exec(code, namespace)
                else:
    ...
    print(f"  Success rate: {passed/(passed+failed)*100:.1f}%" if (passed+failed) > 0 else "  No executable blocks")
...
def main():
    ...
    parser.add_argument("--all", action="store_true", help="Test all documentation files")
