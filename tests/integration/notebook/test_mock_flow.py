...
class TestElementDiscovery:
    ...
    def test_markdown_elements(self, mock_client_class, output_dir):
        ...
        with open(output_dir / "markdown_elements.json", "w") as f:
            json.dump({
                ...
                "all_attributes": [attr for attr in dir(response) if attr.endswith('_elements')]
            }, f, indent=2)
    ...
    def test_all_element_types(self, mock_client_class, output_dir):
        ...
        response = create_mock_response(
            ...
            dataframe_elements=[{"type": "DfElement", "table": pd.DataFrame({"a": [1]})}],
            graph_elements=[{"type": "GraphElement", "data": {}}],
            code_elements=[{"type": "CodeElement", "code": "print('hello')", "language": "python"}],
            image_elements=[{"type": "ImageElement", "url": "http://example.com/img.png"}],
        ...
        with open(output_dir / "all_element_types.json", "w") as f:
            json.dump({
                ...
                "elements_found_in_lui": {
                    ...
                    "types": list(set(e['type'] for e in all_elements)),
        ...
        print(f"Element types found: {set(e['type'] for e in all_elements)}")