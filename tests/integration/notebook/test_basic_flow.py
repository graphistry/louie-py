...
class TestBasicFlow:
    ...
    def test_dataframe_query(self, output_dir):
        ...
        # Query for data
        response = lui("Create a sample dataframe with 5 rows of sales data including columns: product, quantity, price")
                ...
                "dataframe_elements": [
                    {
                        ...
                        "shape": elem.get("table").shape if hasattr(elem.get("table", None), "shape") else None
                    }
                    for elem in response.dataframe_elements
                ],
                "element_keys": list(response.dataframe_elements[0].keys()) if response.dataframe_elements else []
            }, f, indent=2, default=str)
    ...
    def test_traces_enabled(self, output_dir):
        ...
        with open(output_dir / "traces_response.json", "w") as f:
            json.dump({
                ...
                "has_traces": hasattr(response, 'trace_elements') or hasattr(response, 'reasoning_elements')
            }, f, indent=2, default=str)
    ...
    def test_element_types_discovery(self, output_dir):
                ...
                with open(output_dir / f"element_discovery_{i}.json", "w") as f:
                    ...
                    for attr in element_attrs:
                        ...
                        if elements:
                            element_info[attr] = [
                                {
                                    "keys": list(elem.keys()) if isinstance(elem, dict) else None,
                                    "type": elem.get("type", None) if isinstance(elem, dict) else None,
                                    "sample": str(elem)[:200]
                                }
        ...
        with open(output_dir / "element_types_summary.json", "w") as f:
            json.dump({
                ...
                "notes": "These are all the *_elements attributes found across different queries"
            }, f, indent=2)
