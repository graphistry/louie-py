...
class TestOrgAuthFlow:
    ...
    def test_direct_client_org_name_flow(self):
        ...
        with patch('graphistry.pygraphistry.GraphistryClient', return_value=mock_graphistry):
            client = LouieClient(
    ...
    def test_auth_manager_org_passthrough(self):
        ...
        assert len(register_calls) == 1, "Expected exactly one call to graphistry.register()"
        
        register_kwargs = register_calls[0]
        assert 'org_name' in register_kwargs, "org_name should be passed to graphistry.register()"
        assert register_kwargs['org_name'] == target_org, \
            f"Expected org_name='{target_org}', got '{register_kwargs['org_name']}'"

    @pytest.mark.skip(reason="This test demonstrates the confused deputy problem - will fail until fixed")
