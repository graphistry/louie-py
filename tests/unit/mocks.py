...
class MockThread:
    ...
    def __init__(self, thread_id: str = "D_test001", name: Optional[str] = None):
        self.id = thread_id
...
def determine_response_type(prompt: str) -> str:
    ...
    if any(word in prompt_lower for word in ['dataframe', 'query', 'data', 'table', 'sql']):
        return 'dataframe'
    elif any(word in prompt_lower for word in ['graph', 'visualiz', 'network', 'graphistry']):
        return 'graph'
