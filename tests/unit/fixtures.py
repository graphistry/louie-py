...
from .mock_responses import MockResponseLibrary, ResponseScenarios, create_mock_api_response
...
def mock_simple_response():
    ...
    return MockResponseLibrary.text_response(
        "This is a simple response to your query."
    )
...
def mock_streaming_client():
    ...
    def mock_add_cell(thread_id, prompt, agent="LouieAgent"):
        ...
        response.dataframe_elements = [e for e in elements if e.get("type") == "DfElement"]
        response.graph_elements = [e for e in elements if e.get("type") == "GraphElement"]
        response.error_elements = [e for e in elements if e.get("type") == "ExceptionElement"]
